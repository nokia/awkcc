...
fa *mdfa(p, anchor)	/* returns dfa for tree pointed to by p */
	...
	Node *p;
{
	...
	case OR:
		...
		break;
	default:
		awkerr("unknown type %d in penter\n", type(p));
		break;
	}
}
	...
	register Node *p;	/* returns 0 or 1 depending on whether p matches empty string */
{
	...
	awkerr("unknown type %d in first\n", type(p));
	return(-1);
}
	...
	uchar *p;
{
	...
	*/
	...
	if (rtok == '\0')
		...
		awkerr("syntax awkerr in regular expression %s at %s", lastre, prestr);
}
...
Node *primary()
{
	...
	switch (rtok) {
	case CHAR:
		np = op2(CHAR, (Node *) 0, (Node *) rlxval);
		rtok = relex();
		return (unary(np));
	case ALL:
		rtok = relex();
	...
	case CCL:
		np = op2(CCL, (Node *) 0, cclenter(rlxstr));
		rtok = relex();
		return (unary(np));
	case NCCL:
		np = op2(NCCL, (Node *) 0, cclenter(rlxstr));
		rtok = relex();
		return (unary(np));
	case '^':
		rtok = relex();
		return (unary(op2(CHAR, (Node *) 0, (Node *) HAT)));
	case '$':
		rtok = relex();
